...
export const CONFIG = {
    ...
    requestTimeout: 60000,            // Таймаут запроса к API (60 секунд)
    ...
    statAliases: {
        'разум': 'sanity', 'рассудок': 'sanity', 'ум': 'sanity', 'интеллект': 'sanity', 'ментальность': 'sanity',
        'воля': 'will', 'сила воли': 'will', 'решимость': 'will',
        'скрытность': 'stealth', 'незаметность': 'stealth', 'тайность': 'stealth',
        'влияние': 'influence', 'авторитет': 'influence', 'убеждение': 'influence', 'харизма': 'influence'
    },
    ...
    prompts: {
        // --- 1. РОЛИ И НАСТРОЕНИЕ ---
        system: {
            main: `2026 год. Москва. Война с Украиной эхом отдаётся в новостях, а безумные законы душат ЛГБТ-активистов и 'сатанистов'. Ложа Московского O.T.O. — убежище для инакомыслящих, но под угрозой. ФСБ плетёт интриги: вербуют, запугивают, арестовывают. Цель — закрыть Орден через разобщённость, страх, предательство. Игра о моральных дилеммах, где воля ломается, люди отрекаются от идеалов Телемы из страха, выгоды, обид. Но есть те, кто держится — немногие, кто не предаст Орден и себя. Вы — молодой Минервал, в порядке исключения допущенный голосованием с перевесом всего в один голос (голос Мастера Ложи Брата Марсия). Ваша роль: выбрать кем стать, чью сторону принять, кого предать, что обрести — свободу или тюрьму? Когда грань между злом и добром стирается, а ориентиры тонут во мраке, на кону — судьба Ложи, жизни её Членов и ваша собственная.
            Стиль нарратива: иммерсивный, параноидальный, напряжённый, с элементами психологического триллера, конспирологии, сексуальной магии OTO, шепотами о предательстве и видениями Кроули.
            Фокус: Посвятительный путь в Ложе, формирующий личность через выборы: лояльность vs. выгода, воля vs. страх, ритуалы vs. слежка.
            Атрибуты: моральные дилеммы, интриги в Братстве (вербовка ФСБ, саботаж, разоблачения), сенсорные детали (холод московских переулков, гул сирен, вкус запретных вин в ритуалах). Размышления и воспоминания - как якоря в хаосе.`,
            
            scenarioWriter: `Ты — Сценарист квестов. Твоя задача — сгенерировать сложную, многослойную начальную сцену в валидном JSON.`,
            
            simpleTest: `Ты — AI-ассистент для теста связи.`,
        },

        // --- 2. ИНСТРУКЦИИ ФОРМАТА ---
        format: {
            mainTaskPrefix: `ТВОЯ ЗАДАЧА:`,
            
            mainTask: `1. Продолжить сюжет квеста по требованиям.
            2. Новая сцена (10-30 предложений) на основе выбора, в стиле паранойи/интриг/триллера/детектива/драмы, без заголовков.`,

            statAndProgressLogic: `3. Изменения Характеристики, Личности, Прогресса (см.требования).
            4. При достаточном продвижении — дай выбор действий для инициатического Ритуала достижения следующей Степени — начислив после Ритуала очки прогресса.
            5. Рефлексию добавляй в ответ, если уместно.`,
            
            choicesFormat: `6. 5-10 вариантов действий, с [указанием влияния выбора на значения характеристик, если они менятся].`,

            jsonFormatStrict: `ОТВЕТ В ВИДЕ ЗАПОЛНЕННОГО ПО ФОРМАТУ JSON на русском.
            ФОРМАТ ОТВЕТА — СТРОГО JSON: Никакого текста до или после. Не используй markdown. ОТВЕЧАЙ ТОЛЬКО ВАЛИДНЫМ JSON — начинай сразу с { и заканчивай }:`,

            jsonFewShot: `ПРИМЕР JSON СТРУКТУРЫ:
            {
              "scene": "Текст сцены (пример: Марсий достает револьвер. Его рука дрожит...).",
              "short_summary": "ОДНО предложение о ключевом событии сцены (пример: Герой встретился с Марсием, но тот вытащил оружие).",
              "reflection": "Рефлексия (если уместно, пример: Когда вокруг тьма...)",
              "choices": [
                "Вариант 1 [Воля ±n, ...]",
                "Вариант 2 [Разум ±n, ...]",
                ...
              ...
              "progress_change": ±n,
              "start_ritual": "N°",
              "personality_change": "Обновление личности (если изменилась, пример: Минервал, ставший более осторожным)",
              "thoughtsOfHeroResponse": ["Мысль 1", "Мысль 2", ...],
              "inventory": ["Ключ от Ложи", "Странный символ"], 
              "npc_status": {"marsyas_trust": 50} 
            }`,
            
            summaryAndMemoryInstructions: `СИСТЕМА ДАННЫХ И ПАМЯТИ (КРИТИЧНО):
            1. "short_summary": ОБЯЗАТЕЛЬНО. Напиши ОДНО предложение в прошедшем времени, кратко описывающее ключевое событие ЭТОЙ новой сцены. Это пойдет в летопись.
            2. ДИНАМИЧЕСКИЕ ПОЛЯ: Ты можешь добавлять ЛЮБЫЕ новые поля в корневой JSON (например "inventory", "quest_state", "npc_status"), чтобы запомнить факты.`,

            progressAndDegrees: `ВАЖНО — ПРОГРЕСС И СТЕПЕНИ ПОСВЯЩЕНИЯ:`
        },

        // --- 3. ИГРОВЫЕ ИНЪЕКЦИИ ---
        injections: {
            coreMovement: `НЕ ЗАБЫВАЙ: Двигай сюжет. Каждая сцена должна привносить новое.`,

            antiLoop: `ВНИМАНИЕ: СЮЖЕТ ПОВТОРЯЕТСЯ ИЛИ ЗАСТАИВАЕТСЯ.
            ТВОЯ ЗАДАЧА: НЕМЕДЛЕННО введи резкое, ВНЕЗАПНОЕ внешнее событие (облава, взрыв, звонок от ФСБ, новое шокирующее появление персонажа), которое выбивает героя из текущей локации/ситуации и полностью меняет контекст.`,

            twist: `ТРЕБОВАНИЕ (ОБЯЗАТЕЛЬНО В ЭТОЙ СЦЕНЕ): СЮЖЕТНЫЙ ПОВОРОТ.
            Произойдет что-то НЕОЖИДАННОЕ: шокирующее откровение, предательство со стороны близкого, раскрытие скрытой угрозы, кардинальное изменение обстановки. Удиви игрока!`,

            insanity: `СОСТОЯНИЕ ГЕРОЯ: ГРАНЬ БЕЗУМИЯ (Разум КРИТИЧЕСКИ НИЗОК).
            Текст сцены и вариантов должен стать рваным, иррациональным, параноидальным. Вплетай галлюцинации, обрывки фраз из книг Кроули, тени оживают, звуки искажены. Стираются границы между реальностью и вымыслом.`
        },

        // --- 4. ШАБЛОНЫ КОНТЕКСТА ПОЛЬЗОВАТЕЛЯ ---
        userHeaders: {
            d10PromptPart: `d10: `, 
            stateParametersPrefix: `ПАРАМЕТРЫ:`, 
            historyPrefix: `ПРЕДЫСТОРИЯ:`, 
            contextGlobal: `== ГЛОБАЛЬНАЯ ЛЕТОПИСЬ (ВАЖНЫЕ СОБЫТИЯ) ==`,
            contextMemory: `== ТВОЯ ДИНАМИЧЕСКАЯ ПАМЯТЬ МИРА ==`, 
            contextShort: `== КРАТКОСРОЧНАЯ ИСТОРИЯ (ПОСЛЕДНИЕ 3 ХОДА) ==`, 
            currentScene: `ТЕКУЩАЯ СЦЕНА:`,
            action: `ДЕЙСТВИЕ ИГРОКА:`,
            reqThoughts: `\n\nДОПОЛНИТЕЛЬНО: Сгенерируй 10 фраз с мыслями текущей личности героя о своём пути и Телеме.`, 
            reqJsonEnd: `Сгенерируй JSON.` 
        },

        // --- 5. ТЕХНИЧЕСКИЕ ---
        technical: {
            jsonRepair: `СИСТЕМНАЯ ОШИБКА: Твой предыдущий ответ был НЕВАЛИДНЫМ JSON. 
            ПОВТОРНО СГЕНЕРИРУЙ ОТВЕТ СТРОГО ВАЛИДНЫМ JSON. 
            Не включай никакой текст до или после объекта JSON. Только объект.`,
            testMessage: `Проверь соединение и ответь словом "ОК".`,
            testSelf: `Представься в одном коротком предложении, как большая языковая модель.`
        }
    },
    ...
    ФОРМАТ ОТВЕТА (СТРОГО JSON):
    {
      ...
      "thoughtsOfHeroResponse": ["Мысли героя о Марсии", "Страх перед законом", "Восхищение Волей"]
    }`
};
