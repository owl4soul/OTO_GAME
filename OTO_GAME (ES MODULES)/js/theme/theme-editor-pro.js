...
export class ThemeEditorPro {
    ...
    _render() {
        ...
        this._updateNav();
        setTimeout(() => { this._renderPanelContent();
    ...
    _createFooter() {
        ...
        cancel.onclick = () => { themeManagerPro.cancelChanges();
            this.close(); };
        
        const save = document.createElement('button');
        save.textContent = '✓ Сохранить тему';
        save.className = 'te-footer-btn primary';
        save.onclick = () => { themeManagerPro.saveChanges();
            this.close(); };
    ...
    _makeTextInput(val, path) {
        ...
        input.oninput = e => { themeManagerPro.updateSetting(path, e.target.value, false);
            this._updatePreview(); };
        input.onchange = e => { themeManagerPro.updateSetting(path, e.target.value);
            this._pushHistory(); };
        return input;
    }
    ...
    _makeSelect(val, path, options) {
        ...
        sel.onchange = e => { themeManagerPro.updateSetting(path, e.target.value);
            this._pushHistory(); };
        return sel;
    }
    ...
    _renderIconBlock(container, val, path) {
        ...
        sel.onchange = e => { themeManagerPro.updateSetting([...path, 'set'], e.target.value);
            this._pushHistory(); };
        r1.append(l1, sel);
        ...
        inp.type = 'text';
        inp.value = val.emojiFilter || 'none';
        ...
        inp.onchange = e => { themeManagerPro.updateSetting([...path, 'emojiFilter'], e.target.value);
            this._pushHistory(); };
        r2.append(l2, inp);
    ...
    _updateNav() {
        ...
        if (u) { u.disabled = this.historyIndex <= 0;
            u.style.opacity = u.disabled ? 0.35 : 1; }
        if (r) { r.disabled = this.historyIndex >= this.historyStack.length - 1;
    ...
    _renderMockScene(target) {
        ...
        p.id = 'personalityBlockContainer';
